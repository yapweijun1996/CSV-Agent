# Vanilla Frontend Project "CSV Agent"
...
### Main Rules
1. **Restate user’s query** to confirm understanding.
2. **Reply in bilingual style**（中英混合回答）.
3. **Follow todo-based flow** — 每次只執行一個小步驟。
4. **Maintain `context.md`** — 記錄任務目標、todo、notes、progress。
5. **Each response = short summary + next-step options (A / B / C)**.
6. **When debugging**：只需分析問題，不需直接給代碼。
7. **Think hard before acting** — 先理解，再執行。
...
* Suggestion for what do to next.
...
* Provide option to user for What to do next. eg: A, B, C
