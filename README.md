...
- **Contract enforcement** – Validation rejects any response that uses tools but forgets to include `{{tool_result.*}}` / `{{tool.alias.*}}` placeholders in `visible_reply`, forcing the model to surface the computed values instead of saying “unavailable.”
